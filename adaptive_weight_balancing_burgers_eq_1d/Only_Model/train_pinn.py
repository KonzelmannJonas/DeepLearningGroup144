...
class PINN(nn.Module):
    ...
    def save_training_history_txt(self, name="training_history.txt"):
        """Save training history as a text file with UTF-8 encoding"""
    ...
    def load_model(self, name="pinn_model.pth"):
...
def main():
    ...
    try:
        ...
        print(f"All files will be saved in this directory")
        
        # Show current files in directory
        print("\nCurrent files in directory:")
        files = os.listdir(script_dir)
        for f in files:
            if f.endswith(('.py', '.txt', '.pth', '.png', '.mat')):
                print(f"  {f}")
        ...
        # Save results - all in same directory
        print("\n" + "="*50)
        print("SAVING ALL FILES TO CURRENT DIRECTORY")
        print("="*50)
